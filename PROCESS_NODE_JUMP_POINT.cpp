...
PROCESS_NODE_JUMP_POINT::PROCESS_NODE_JUMP_POINT(CONTAINER* c, STATIC_FONT* font, DRAWER::DRAWERS_SHADER* lineDrawers, PROCESS_NODE_LINER* processNodeLiner, PROCESS_NODE_MANAGER* processNodeManager):
	ProcessNodeManager(processNodeManager),
	StringBoxInputJumpPoint(c,font,processNodeManager),
	...
	NodeTypeNameUi = UI_FACTORY::instance(DRAWER::STATIC_FONT::instance("ジャンプポイント", font, VECTOR2(), VECTOR2(24.0f, 24.0f), DRAWER::STATIC_FONT::drawMethodDefault()));
...
PROCESS_NODE_JUMP_POINT::~PROCESS_NODE_JUMP_POINT() {
	disconnect(this);
}

std::pair<PROCESS_NODE_CONNECTOR_IN**, int> PROCESS_NODE_JUMP_POINT::allNodeConnectorIn() {
	return { &NodeConnectorFront_, 1 };
}

std::pair<PROCESS_NODE_CONNECTOR_OUT**, int> PROCESS_NODE_JUMP_POINT::allNodeConnectorOut() {
	return { &NodeConnectorBack_,1 };
}

std::string PROCESS_NODE_JUMP_POINT::saveDataString() {
	return PROCESS_NODE_SAVE::commonSaveDataString(PROCESS_NODE_SAVE::getTypeStringName(PROCESS_NODE_TYPE::JUMP_POINT), rootUi()->pos()) +
		"jump_point{" + StringBoxInputJumpPoint.string() + "}";
}

void PROCESS_NODE_JUMP_POINT::loadFromSaveString(const STRING_TREE& data) {
	rootUi()->setPos(PROCESS_NODE_LOAD::loadSaveDataStringPos(data));
	int idx = data.findData("jump_point");
	if (idx != -1) {
		StringBoxInputJumpPoint.setString(data.childs()[idx]->data());
	}
}

void PROCESS_NODE_JUMP_POINT::compile(ROBOT_COMPILER* robotCompiler){
	robotCompiler->addJumpPoint(StringBoxInputJumpPoint.string(), this);
}

void PROCESS_NODE_JUMP_POINT::nextStep(ROBOT_PROCESSOR* robotProcessor) {
...
void PROCESS_NODE_JUMP_POINT::setEnable(){
	StringBoxInputJumpPoint.setEnable();
}

void PROCESS_NODE_JUMP_POINT::setDisable(){
	StringBoxInputJumpPoint.setDisable();
}
