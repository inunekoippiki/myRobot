...
void SOUND_MANAGER::update(){
	...
	if (NewBgm != nullptr) {
		NewBgmVolume += FeedSpped * 2.0f;
		NowBgmVolume -= FeedSpped;
...
void SOUND_MANAGER::playSound(SOUND* sound, const VECTOR3& pos){
	sound->setVolume(cvt(unCvt(_SEVolume) / (1.0f + length(PlayerPos - pos)* Attenuation)));
	sound->play();
}
...
void SOUND_MANAGER::setVolumeSE(SOUND* sound){
	sound->setVolume(_SEVolume);
}

void SOUND_MANAGER::setVolumeBGM(SOUND* sound){
	sound->setVolume(_BGMVolume);
}
...
void SOUND_MANAGER::load(SETTING* setting){
	setMasterVolume(setting->volume()->percentage());
	setSEVolume(setting->soundVolume()->percentage());
	setBGMVolume(setting->bgmVolume()->percentage());
	if (NowBgm != nullptr) {
		NowBgm->setVolume(_BGMVolume);
	}
	if (NewBgm != nullptr) {
